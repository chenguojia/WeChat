...
import com.cardvalue.wechat.repository.MerchantRepositoryCustom;
...
public class MerchantRepositoryImpl implements MerchantRepositoryCustom{
	...
	public int create(MerchantRegisterDTO dto, Sales sales) {
		...
		if(count == 0){
			...
			/*//复制属性
			UserInfo tempUserInfo = new UserInfo();
			tempUserInfo.setMobile(dto.getContactMobile());
			tempUserInfo.setName(dto.getContactName());
			tempUserInfo.setUserId(user.getId());
			tempUserInfo.setProvinceId(dto.getProvinceId());
			tempUserInfo.setRegionId(dto.getRegionId());
			tempUserInfo.setCreateDate(new Date());
			if(dto.getProvinceId() > 0){
				tempUserInfo.setProvinceName(utilRepository.getProvincesOrRegionsById(dto.getProvinceId()).getName());
			}
			if(dto.getRegionId() > 0){
				tempUserInfo.setRegionName(utilRepository.getProvincesOrRegionsById(dto.getRegionId()).getName());
			}
			*//**创建userInfo对象*//*
			UserInfo userInfo = userInfoRepository.save(tempUserInfo);
			logger.info("成功创建userInfo对象，id为："+ userInfo.getId());*/
			...
			/*List<KeyMerchantDTO> keyMerchants = keyMerchantRepo.findByMid(merchant.getMid());
			KeyMerchantDTO km = keyMerchants.get(0);
			merchant.setBranch(km.getProvince());
			merchant.setPrefecture(km.getPrefecture());*/
