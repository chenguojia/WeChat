   ...
   <%-- <div class="panel-heading">
        ...
        <form name="form1" class="clearfix" id="form1" action="<c:url value="/report/exportReport"/>"
            ...
            <div class="input-group" style="float: right">
                &lt;%&ndash; <button class="btn btn-primary showModalBtn" type="button" url="<c:url value="/background/${moduleName}/new" />" style="margin-right: 5px;">新增</button>
                 <button class="btn btn-danger batchDellBtn " type="button" url="<c:url value="/background/${moduleName}/new" />" parentUrl="<c:url value="/background/${moduleName}/init"/>">删除</button>&ndash;%&gt;
            </div>
        </form>
    </div>--%>
...
<script>
    ...
    function initPagination() {
        ...
        function InitTable(pageIndex) {
            ...
            $.ajax({
                type: "get",
                url: "<c:url value="/background/${moduleName}/list?currPageNum="/>" + pageNum + "&targetLabelId=${targetLabelId}&targetValueId=${targetValueId}",
                async: false,
                cache: false,
