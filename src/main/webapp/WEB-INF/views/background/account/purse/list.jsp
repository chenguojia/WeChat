...
<div class="panel-heading">
    ...
    <form name="form1" class="clearfix" id="form1" action="<c:url value="/report/exportReport"/>"
        ...
        <div class="input-group" style="float: right">
            <button class="btn btn-primary batchPassBtn " style="margin-right: 10px;" type="button"  parentUrl="<c:url value="/background/${moduleName}/init"/>" status="1">通过</button>
            <button class="btn btn-default exportPurseBtn" type="button" parentUrl="<c:url value="/background/${moduleName}/export"/>" >导出</button>
        </div>
    </form>
</div>
...
<div class="panel-body">
    ...
    <div class="table-responsive" style="max-height:300px;overflow-y:scroll">
        <table class="table table-hover" cellpadding="0" cellspacing="0" border="0" id="myTable" style="width:950px;">
...
<script>
    ...
    $(".batchPassBtn").click(function(){
        ...
        if (ids == "") {
            layer.alert("请至少选中一个需审批通过的条目!",0);
            return;
        } else {
            approve(ids,status,'',parentUrl);
        }
    });

    //审批公用
    function approve(ids,status,reason,parentUrl){
        $.ajax({
            type: "POST",
            url:"<c:url value="/background/${moduleName}/approve"/>",
            data:{"ids":ids,"status":status,"reason":reason},
            cache: false,
            success: function(data) {
                if (data == "1") {
                    //成功
                    ...
                    //失败
                    layer.alert("服务器繁忙，操作失败!" , 8);
                }
            }
        });
    }

    $(".exportPurseBtn").click(function(){
        window.location.href = $(this).attr("parentUrl");
    });
