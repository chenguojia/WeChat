...
<tr class="label-primary">
    ...
    <td scope="col" width="40%"><span>关键字</span></td>
    <td scope="col" width="20%"><span>优先级</span></td>
    <td scope="col" width="20%"><span>类型</span></td>
...
<c:forEach var="item" items="${queryPageModel.list}">
    <tr>
        ...
        <td><a class="showModalBtn" url="<c:url value="/background/${moduleName}/view?id=" />${item.id}&optType=1" href="#">${item.keyword}</a></td>
        <td>
            <c:if test="${item.priority == '0'}">低</c:if>
            <c:if test="${item.priority == '1'}">中</c:if>
            <c:if test="${item.priority == '2'}">高</c:if>
        </td>
        <td>
            <c:if test="${item.responseType == 'text'}">文字消息</c:if>
            <c:if test="${item.responseType == 'news'}">图文消息</c:if>
            <c:if test="${item.responseType == 'function'}">特殊逻辑</c:if>
        </td>
        <td>
