...
<tr class="label-primary">
    ...
    <td scope="col" width="80%"><span>用户类型</span></td>
...
<c:forEach var="item" items="${queryPageModel.list}">
    <tr>
        ...
        <td>
            <a class="showModalBtn" url="<c:url value="/background/${moduleName}/view?id=" />${item.id}&optType=1" href="#">
                <c:if test="${item.userType == '0'}">其他</c:if>
                <c:if test="${item.userType == '1'}">银商客经</c:if>
                <c:if test="${item.userType == '4'}">商户</c:if>
            </a>
        </td>
        <td>
